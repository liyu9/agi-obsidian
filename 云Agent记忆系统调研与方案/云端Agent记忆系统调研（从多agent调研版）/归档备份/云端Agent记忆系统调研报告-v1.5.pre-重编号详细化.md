...
> **能力维度是记忆系统命脉**：本方案为**多知识库架构**（个人/部门/公司多库分担负载，见 §6.4.1 ④层），2000 人高频场景下各实例按隔离粒度承载相应负载；能否把长对话自动压缩为按需加载的分层摘要，决定 token 成本与召回质量。OpenViking 是 4 款中唯一具备"transcript→commit→archive→L0/L1/L2 完整会话压缩管线 + 三层信息结构 + Memory/Resources/Skills 三类上下文统一"三合一能力的产品；Cognee 以知识图谱为核心（remember→图谱），无分层压缩归档管线，更适合作为关系推理增强层而非独立承担会话记忆。
>
...
│                                                                  │
│  〔接入方式 · 正交维度〕CC→Hooks+MCP │ OpenClaw→MCP/SDK（择一）  │
│     注：接入方式是 Agent 出站协议选择，非数据流串行站            │
└─────────────────────────────┬───────────────────────────────────┘
...
> 接入方式详细说明（三通道职责 + 7 Hook 生命周期时间轴）见下方二期架构图。
...
│                                                                  │
│  〔接入方式 · 正交维度·非串行站〕                                 │
│   Plugins(Hooks) │ MCP │ SDK/HTTP ——Agent 按能力择一，非依次经过 │
│   7 Hook 覆盖读/写/归档/隔离 │ 9 工具 search/read/store/... │ 通用编程接入 │
└─────────────────────────────┬───────────────────────────────────┘
                              │ 入站消息携带 Agent 身份（mTLS/JWT）
                              ▼
┌─────────────────────────────────────────────────────────────────┐
...
**接入方式正交维度·详细说明**（一期/二期通用 · Agent 按能力择一，非依次经过）

接入方式是 ① Agent 出站的协议选择维度，与数据流主干（①→③→④）正交：同一请求按 Agent 框架能力择一走 Hooks / MCP / SDK 通道，而非依次经过。三通道职责与适用场景：
...
| SDK/HTTP | 通用编程接入 | Python/Go/TS SDK + HTTP | 无 MCP/Hook 能力的框架（LangChain/二期自建/特殊集成） |
...
#### 6.4.2 层职责说明
数据流主干为 ①→③→④（⑤横切旁路）。接入方式（Hooks/MCP/SDK）是 ① Agent 出站的正交维度，Agent 按能力择一、非依次经过，不单独成层。

| 层                 | 承担职责                                                                                                                     | 关键组件                                                          | 归属         |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------- | ---------- |
| ① Agent 应用层       | 消费记忆能力，产生对话 / 任务 / 事实；Agent 无库归属，被授权访问知识库；**接入方式为正交维度**（Hooks/MCP/SDK 择一，见下方接入范式）                | Claude Code、OpenClaw、Hermes、Cursor/Trae（外部 MCP 兼容）、二期自建 Agent | 官方接入对象     |
...
> 4 条路径对应接入方式（正交维度）的两条通道（Plugins/Hooks + MCP）× 两个方向（写入 + 召回）。Hooks 通道自动触发，MCP 通道模型按需触发。
