...
本报告面向 2000 人规模互联网公司**统一部署多类 AI Agent**（Claude Code / OpenClaw / Hermes 等）的记忆共享与治理需求。核心结论：以「知识库」为中心抽象、**OpenViking** 为底座、叠加多实例物理隔离与②层知识库授权中心，实现跨 Agent 记忆复用、部门/个人/会话三级隔离与企业级合规治理。
...
**目标形态**：为云端 Agent 挂载**以「知识库」为中心的记忆系统**——知识库按归属与用途分类（个人偏好库 / 个人技能库 / 项目上下文库 / 部门 SOP 库 / 公司政策库 / 会话归档库，详见 §6.4.3），Agent 不拥有知识库、只被授权访问。具体具备：

- **按库分类承载**：短期（会话内）+ 跨会话工作 + 长期稳定 + 归档冷存，分别落在对应类型的知识库
- **隔离分两层**：
  - **规范库（逻辑隔离）**：个人偏好/技能/会话归档等，共享实例内以 namespace 隔离
  - **实例库（物理隔离）**：部门 SOP、公司政策等高敏/只读库，用独立 OpenViking 实例隔离（多实例物理隔离，见 §6.4.0）
- **迁移与授权**：一个 Agent 可访问多个知识库（跨库联邦查询，见 §6.4.0）；**一期按 1 实例↔1 Agent↔1 库的最简映射（多 Agent 并存、各自 1-to-1）先跑通**，二期扩展为 1 Agent↔N 库 + 多实例池 + 跨库联邦（见 §6.4.0 分期规划）
- **企业级治理**：Agent×知识库 授权矩阵 + ACL + 审计 + 合规 + 可观测（见 §6.4.1 ②层）

> 具体的落地模式与产品选型延后到 §5 候选调研与 §6 业务侧选型与方案设计，本章不预设结论。
...
| 参考对象               | 一手事实                                                                                                                                                                                          | 可迁移原则                           |
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------- |
| Claude Code Memory | 官方 **4 层**：Enterprise policy（组织级只读）+ Project memory（`./CLAUDE.md`）+ User memory（`~/.claude/CLAUDE.md`）+ Project memory local（`./CLAUDE.local.md`，已 Deprecated 建议改用 imports）；提供 `/memory` 编辑入口 | 企业场景需要**组织级只读 + 项目级 + 用户级**分层承载（本方案对应 §6.4.3 六类知识库：公司政策库=组织级只读、部门SOP库=项目级、个人偏好/技能库=用户级） |
...
| 权限模型 | 访问控制 ACL | Read / Write / Delete / Share 细粒度权限（能力上限，如 Cognee dataset 粒度 4 类 ACL）；本方案据 Agent 意图驱动特性降为布尔粒度（允许/不允许），详见 §6.4.3 |
...
>
> 以上完成 4 款开源候选的能力调研与横向对比。下文 §6 基于此调研结论，结合 §1 业务约束与 §3 业务题，给出业务侧选型推荐与方案设计。
...
> **能力维度是记忆系统命脉**：本方案为**多知识库架构**（个人/部门/公司多库分担负载，见 §6.4.1 ③层（多知识库实例池）），2000 人高频场景下各实例按隔离粒度承载相应负载；能否把长对话自动压缩为按需加载的分层摘要，决定 token 成本与召回质量。OpenViking 是 4 款中唯一具备"transcript→commit→archive→L0/L1/L2 完整会话压缩管线 + 三层信息结构 + Memory/Resources/Skills 三类上下文统一"三合一能力的产品；Cognee 以知识图谱为核心（remember→图谱），无分层压缩归档管线，更适合作为关系推理增强层而非独立承担会话记忆。
>
...
> **选型结论**：本报告选定 OpenViking 作为云端 Agent 记忆系统的主选型。下文 §6.3 起围绕此选型展开业务场景覆盖与方案架构设计。
...
基于 §6.2 选型结论，OpenViking 作为主选型。本节从业务场景覆盖与分期价值角度说明方案意义，技术架构见 §6.4。

**对齐 §2 三类用户诉求**：
- A · Agent 使用方（个人/知识库维护方）：§6.4.3 六类知识库承载个人偏好/技能与部门 SOP
- B · 公司/部门（管理视角）：§6.4.1 ②层授权矩阵 + §6.4.3 知识库归属分级隔离
- C · 云端 Agent 团队（建设方）：§6.4.1 三层架构 + §6.4.4 接入方式 + §6.5 方案级风险

**业务场景与分期满足度**

本方案按两期演进，分期满足不同层次的业务场景（对齐 §1 痛点、§3 五道业务题 Q1-Q5）：

| 场景 | 对应业务题 | 一期是否满足 | 二期是否满足 | 说明 |
|---|---|---|---|---|
| 单 Agent 跨会话记忆（同一用户明天再来不用重讲） | Q2 短期+长期闭环 | ✅ | ✅ | 一期 1 Agent↔1 库即跑通记忆沉淀与召回 |
| 个人偏好/技能持久化（记住用户习惯、习得操作） | Q2 | ✅ | ✅ | Hooks 自动 capture + Memory Extraction 落地 |
| 会话归档与冷存（长对话压缩归档、180天迁 TOS） | Q2 | ✅ | ✅ | 单实例内 commit→archive→TOS 链路完整 |
| 单 Agent 接入鉴权与审计（谁访问了什么） | Q4 治理 | ✅ 部分 | ✅ | 一期②层简化版有鉴权+审计，无授权矩阵 |
| 多 Agent 并存各自隔离（CC/OpenClaw 互不串数据） | Q3 三级隔离 | ✅ 天然 | ✅ | 一期各 Agent 独占实例天然物理隔离 |

**二期的业务价值**

2000 人规模互联网公司统一部署多类 AI Agent，知识库类型多（个人偏好/技能、项目上下文、部门 SOP、公司政策、会话归档等 6 类，见 §6.4.3），且知识库天然需要被分享与复用。一期"1 实例↔1 Agent↔1 库"解决了单 Agent 记忆链路跑通的基础问题，但无法支撑以下真实业务需求，二期是这些需求落地的必要演进（二期技术形态见 §6.4.0 分期规划）：
...
二期价值：知识库作为独立产品（见 §6.4.3），同一知识库可被多个 Agent 授权访问——公司政策库只存一份、全公司所有 Agent 只读共享；部门 SOP 库部门内 Agent 共享。一处更新、所有授权 Agent 即时生效，知识沉淀可复制可分享，跨 Agent 协作记忆复用（对应 Q1 跨 Agent 共享）。

**需求三 · 知识库类型多，需统一治理与隔离**

2000 人公司知识库类型多、归属杂：个人库（2000 个）、部门库（~20 个）、公司库（1 个）、项目库（按项目数）、会话归档库（按活跃会话）。这些库敏感度不同（个人偏好低敏、部门 SOP 中敏、公司政策高敏只读），需要分级隔离与统一授权治理。一期靠实例物理分隔硬做，无策略层——部门管理者无法管控"本部门知识库被哪些 Agent 访问"，合规审计无统一入口。
...
- **好处**：OpenViking 升级 → ② 层授权中心无需重写（仅靠 HTTP 网关转发对接）；合规责任边界清晰（与 §6.5 风险 2 一致）。
...
> 二期的业务价值（一个 Agent 用多库 / 多个 Agent 用一库 / 多类型库治理 / 零改造接入）见 §6.3。

**演进原则**：②层网关入口一期即部署（简化版），二期补全授权矩阵 + 联邦 + 多实例路由；Agent 侧接入契约不变，二期零改造演进。

**概念归属对照（参考）**

以下架构图与目录表中，每个节点用 `(官方)` / `(企业侧扩展)` 标注。完整对照如下：
...
| **多实例物理隔离**（部门知识库用独立 OpenViking 实例，非官方 account 级逻辑隔离）                  | ⚠️ 企业侧扩展                   | 官方支持 account 级逻辑隔离，本方案选择独立实例做物理隔离（更强）                                           |
...
本方案按两期演进（分期规划见 §6.4.0），以下分别给出一期与二期架构图。一期验证单 Agent 记忆链路可行性，二期面向 2000 人多 Agent 真实场景补全授权矩阵与跨库联邦。
...
**接入方式详细说明**（一期/二期通用 · 与数据流主干分离 · Agent 按能力择一，非依次经过）

接入方式是 Agent 出站时用什么协议与本系统通信的选择，和数据流主干（①→②→③）是两件事：同一请求按 Agent 框架能力择一走 Hooks / MCP / SDK 通道，而非依次经过。接入方式不是数据流的"第二层"，而是 ① Agent 出站时的协议选择维度，详细如下（官方 · 附件4 第 60-110 行）：
...
| MCP | 模型意图驱动·按需调用 | 9 工具：search/read/list/store/add_resource/grep/glob/forget/health | MCP 兼容 Agent（Cursor/Trae/OpenClaw 等） |
...
数据流主干为 ①→②→③。接入方式（Hooks/MCP/SDK）是 Agent 出站时的协议选择，Agent 按能力择一、非依次经过，不单独成层（详见 §6.4.1 接入方式详细说明）。横切支撑层为旁路依赖，不参与层级编号。

| 层                 | 承担职责                                                                                                                     | 关键组件                                                          | 归属         |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------- | ---------- |
| ① Agent 应用层       | 消费记忆能力，产生对话 / 任务 / 事实；Agent 无库归属，被授权访问知识库；接入方式为 Hooks/MCP/SDK 择一（见 §6.4.1 接入方式详细说明）                | Claude Code、OpenClaw、Hermes、Cursor/Trae（外部 MCP 兼容）、二期自建 Agent | 官方接入对象     |
...
| ③ 多知识库实例池         | 个人知识库共享实例（namespace 逻辑隔离）+ 部门知识库独立实例（物理隔离）+ 公司政策知识库只读实例；每实例含 openviking-server（治理+抽取+检索+存储）+ web-studio + ov CLI + RAGFS | 多个 OpenViking 实例 + RAGFS + flat_hybrid + web-studio + ov CLI  | 官方         |
...
| 知识库类型      | 归属方    | OpenViking 命名空间                   | 记忆类型（官方分类）                              | 承载内容                    | 实例形态（多实例物理隔离）          | 数量预估（2000 人） |
| ---------- | ------ | --------------------------------- | --------------------------------------- | ----------------------- | ------------------ | ------------ |
...
> 方案级风险（OpenViking 选型 / 本方案架构）见 §6.5；以下为记忆系统通用风险。

| # | 风险 | 影响 | 缓解 |
|---|------|------|------|
| R1 | 记忆污染（错误偏好被强化） | AI 行为偏离 | 借鉴 Letta 的召回-证据引用做法（archival passage + sleep-time 整合，每次召回要求 LLM 显式引用来源），做记忆污染防护 |
| R2 | 2000 人规模的成本失控 | 算力/存储账单爆 | 引入"记忆生命周期"策略：180 天未访问自动归档到 TOS 对象存储（知识库级归档，对齐 §6.4.3）；并按官方默认关键参数（commit=20000 / recallLimit=6）预算 LLM+Embedding 成本 |
...
- **竞品功能参考被误读为"完整候选"**：Coze / VikingDB / Dify 已从 §6.2 的候选池移出，仅在附件2 承载功能借鉴。选型评审时请复核，不要把附件2 三款商业产品误当作可选项。
...
3. **持续观察竞品功能参考三者向"记忆系统级"抽象的演进**（关联 §6.5 风险1 单产品绑定）（Coze / VikingDB / Dify）；**触发重评估条件**：Coze 补齐部门级 ACL 或 VikingDB 提供长期记忆 SaaS 或 Dify 官方深化 Mem0 集成，任一发生即重开选型
