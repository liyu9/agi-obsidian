...
> **版本**：v1.4（方案3 · 架构图精简 · 删4原语+删Adapter预留+简化授权粒度+横切层重构）
> **发布日期**：2026-07-03
> **面向读者**：AI 产品经理 / 平台工程 / 安全合规 / 部门业务方
> **本次修订摘要**：架构图精简——删 4 原语（remember/recall/update/forget）、删 Adapter 预留（VectorAdapter/GraphAdapter/KVAdapter）、简化授权粒度为布尔（允许/不允许）、⑤ 层重构为横切支撑层、全文 KB→知识库；§6.2-6.3 选型范式修正——4 款开源候选平等参与主选型，经五维度综合判断（能力/知识库/范式体验/一体化/落地约束）OpenViking 胜出；详见文末【修订记录 v1.4】。
...
> 一期与长期均采用 OpenViking 单栈，不预留 Adapter 抽象层（避免过度设计）。OpenViking 经 §6.2.1 五维度综合判断胜出为主选型；Mem0 / Letta / Cognee 落选但保留作能力参考。**Coze / VikingDB / Dify 为商业闭源，不参与选型**，仅在[附录文档](附件2-引用来源与竞品参考.md)中承载功能借鉴。

### 6.4 方案架构（OpenViking 单栈）

#### 6.4.0 设计理念：以知识库为中心 + OpenViking 官方原生 vs 企业侧扩展

> **范式声明（方案3 · 以知识库为中心）**：本方案以"**知识库**"为一等公民，知识库归属组织/部门/个人，**不归属于 Agent**；Agent 是被授权的访问者。一个组织/人可拥有多个知识库（个人偏好知识库、项目上下文知识库、部门 SOP 知识库等），多个 Agent 可被授权访问同一知识库。这是经典 RBAC + 数据权限模型在记忆系统的应用——**Agent 与数据归属解耦**，新增/下线 Agent 只改授权矩阵，不动数据。
>
> **实现方式（方式C · 混合）**：个人偏好/技能/会话知识库 → 共享 OpenViking 实例，按 `viking://user/{uid}/` namespace 隔离 + 授权中心 ACL；部门 SOP 知识库 → 独立实例（按部门或敏感度分组），强物理隔离；公司政策知识库 → 独立只读实例，所有 Agent 授权只读。
>
> **一句话定义（企业侧扩展）**：**企业侧扩展 = 在 OpenViking 官方原生能力之上，由公司（企业侧）自己叠加的一层治理逻辑与目录约定；OpenViking 不提供、不感知、不改源码。** 其中"**知识库授权中心（RBAC + ACL + 审计）**"是企业侧扩展的核心组件，在 Agent 与 OpenViking 多知识库实例池之间做授权决策与路由。
...
- **好处**：OpenViking 升级 → 企业侧扩展无需重写（仅靠 URI 拼装 + 元数据过滤对接）；合规责任边界清晰（与 §6.6 风险 2 一致）。
