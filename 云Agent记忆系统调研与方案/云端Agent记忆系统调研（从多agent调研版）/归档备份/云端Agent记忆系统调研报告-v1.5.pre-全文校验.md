...
本报告面向 2000 人规模互联网公司**统一部署多类 AI Agent**（Claude Code / OpenClaw / Hermes 等）的记忆共享与治理需求。核心结论：以「知识库」为中心抽象、**OpenViking** 为底座、叠加多实例物理隔离与②层授权中心，实现跨 Agent 记忆复用、部门/个人/会话三级隔离与企业级合规治理。
...
> 由 §1 痛点与 §5 候选调研提炼。聚焦两类核心问题，对应 §6 建设方案要回答的设计目标：
...
> 以上两类问题在 §6 建设方案中进一步拆解为 5 道业务题（Q1 跨 Agent 共享 / Q2 短期+长期闭环 / Q3 三级隔离 / Q4 企业级治理 / Q5 可观测）。
...
> 4 条路径对应接入方式的两条通道（Plugins/Hooks + MCP）× 两个方向（写入 + 召回）。Hooks 通道自动触发，MCP 通道模型按需触发。
