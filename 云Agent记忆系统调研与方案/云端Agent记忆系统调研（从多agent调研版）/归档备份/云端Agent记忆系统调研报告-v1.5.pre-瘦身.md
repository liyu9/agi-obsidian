...
- **隔离分两层**：
  ...
  - **实例库（物理隔离）**：部门 SOP、公司政策等高敏/只读库，用独立 OpenViking 实例隔离（方式C，见 §6.4.5）
...
> 以上两类问题在 §6 建设方案中进一步拆解为 5 道业务题（Q1 跨 Agent 共享 / Q2 短期+长期闭环 / Q3 三级隔离 / Q4 企业级治理 / Q5 可观测），其选定理由见 §6.5 核心理由。
...
> **能力维度是记忆系统命脉**：本方案为**多知识库架构**（个人/部门/公司多库分担负载，见 §6.4.1 ④层与 §6.4.5），2000 人高频场景下各实例按隔离粒度承载相应负载；能否把长对话自动压缩为按需加载的分层摘要，决定 token 成本与召回质量。OpenViking 是 4 款中唯一具备"transcript→commit→archive→L0/L1/L2 完整会话压缩管线 + 三层信息结构 + Memory/Resources/Skills 三类上下文统一"三合一能力的产品；Cognee 以知识图谱为核心（remember→图谱），无分层压缩归档管线，更适合作为关系推理增强层而非独立承担会话记忆。
>
