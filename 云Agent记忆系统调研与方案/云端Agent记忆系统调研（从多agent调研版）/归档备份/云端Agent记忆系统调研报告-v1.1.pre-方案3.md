...
> **版本**：v1.1（校验后修订版，基线 v1）
> **发布日期**：2026-07-02
> **面向读者**：AI 产品经理 / 平台工程 / 安全合规 / 部门业务方
> **本次修订摘要**：修复 1 Blocker + 7 Major + 若干 Minor/Nit，详见文末【修订记录】。
...
#### 6.4.0 设计理念：OpenViking 官方原生 vs 企业侧扩展

> **一句话定义**：**企业侧扩展 = 在 OpenViking 官方原生能力之上，由公司（企业侧）自己叠加的一层治理逻辑与目录约定；OpenViking 不提供、不感知、不改源码。**
...
│  ③ 企业侧记忆治理层  (企业侧扩展)                                  │
│  ┌─────────────┬──────────────┬─────────────┬──────────────┐    │
│  │ 接口收敛     │ ID / 元数据  │ Visibility  │ 观测埋点转发  │    │
│  │ 4 业务动词   │ 五级 ID 注入 │ 拼路径+白名单│ OTel + Prom  │    │
│  │             │ account/team │ (企业侧字段) │ +token 成本  │    │
│  │             │ /user/agent/ │             │ llm_token/   │    │
│  │             │ session      │             │ embed_token  │    │
│  └─────────────┴──────────────┴─────────────┴──────────────┘    │
│  ┌──────────────────────────────────────────────────────────┐   │
...
└─────────────────────────────┬───────────────────────────────────┘
                              ...
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│  ④ OpenViking 单栈  (官方 · 附件4 第 17-27 行 六层)               │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │ 核心服务 openviking-server                                │   │
│  │  ├ 记忆治理：ACL（ROOT/ADMIN/USER）+ 事务锁 + audit log  │   │
│  │  ├ 抽取管线：Memory Extraction（LLM，按 MemoryTypeRegistry│   │
│  │  │            5 类分类）+ Working Memory v2 Compression   │   │
│  │  │            （LLM Function Calling，按 WM 7 sections   │   │
│  │  │            结构化压缩，section 级 KEEP/UPDATE/APPEND） │   │
│  │  │            ─ 注：v1 组件名 Parser/TreeBuilder/         │   │
│  │  │              SemanticQueue/SessionCompressorV2/        │   │
│  │  │              MemoryUpdater 已合并入现行管线            │   │
│  │  ├ 分层信息模型：L0 .abstract.md / L1 .overview.md /     │   │
│  │  │                L2 原文（按需拉取省 Token）              │   │
│  │  ├ 检索引擎：flat_hybrid（dense + sparse + int8 量化）   │   │
│  │  │           + doubao-seed-rerank 两阶段                  │   │
│  │  ├ 目录承载：viking://resources / user / session / agent │   │
│  │  └ 底座：RAGFS（Rust）+ VectorDB（dense+sparse 索引）    │   │
│  ├──────────────────────────────────────────────────────────┤   │
│  │ web-studio（Web 控制台 · Playground + 检索轨迹可视化）    │   │
│  ├──────────────────────────────────────────────────────────┤   │
│  │ ov CLI（Rust 命令行 · ls/tree/read/search 批量运维）      │   │
│  └──────────────────────────────────────────────────────────┘   │
└─────────────────────────────┬───────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│  ⑤ 外围支撑                                                       │
│   Prometheus + Grafana（观测）│ KMS（密钥·企业侧扩展）│ TOS（对象存储）│
│   + audit log（合规审计·企业侧扩展）                              │
...
> - ③ 企业侧记忆治理层是本报告设计的"薄治理 SDK"，在官方命名空间之上叠加 ID 注入 + Visibility + 观测转发
> - ④ OpenViking 单栈的治理层 ROOT/ADMIN/USER 是 OpenViking 内置权限模型，与 Claude Code 4 层记忆分层（Enterprise/Project/User/Local）无关

#### 6.4.2 层职责说明

| 层 | 承担职责 | 关键组件 | 归属 |
|---|---|---|---|
| ① Agent 应用层 | 消费记忆能力，产生对话 / 任务 / 事实 | Claude Code、OpenClaw、Hermes、未来 Agent | 官方接入对象 |
| ② 接入通道层 | 三通道接入：Plugins(Hooks) 自动驱动读/写/归档/隔离；MCP 模型按需调 9 工具；SDK/HTTP 通用编程接入 | 7 Hook 脚本 + MCP Server（9 工具）+ Python/Go/TS SDK | 官方 |
| ③ 企业侧记忆治理层 | 在官方 viking:// 4 命名空间之上叠加：接口收敛 + 五级 ID 注入 + Visibility 拼路径 + 观测埋点转发（含 token 成本）；OpenVikingAdapter 一期仅做调用封装 | 薄治理 SDK + OpenVikingAdapter | 企业侧扩展 |
| ④ OpenViking 单栈 | 核心服务（治理 + 抽取管线 + 分层信息模型 + 检索引擎 + 目录承载 + RAGFS 底座）+ web-studio 控制台 + ov CLI | openviking-server + web-studio + ov CLI + RAGFS + flat_hybrid | 官方 |
| ⑤ 外围支撑 | 观测、密钥、对象存储、合规审计、模型层（LLM+Embedding+VLM+rerank） | Prometheus / Grafana / KMS / TOS / audit log / 豆包模型 | 官方 + 企业侧扩展 |
...
> **官方原生命名空间（4 个顶级，附件4 第 28-35 行）**：`viking://resources` / `viking://user` / `viking://session` / `viking://agent`。OpenViking 官方**无 `account/`、`team/`、`archive/` 顶级目录**。
>
> **企业侧扩展（本报告设计）**：在官方 4 命名空间之上叠加 `account/` 多租户根（对应 ROOT/ADMIN/USER 权限模型的账号维度）+ `team/{dept}/` 部门分区 + `archive/` 冷存策略。下表"OpenViking 目录"列以 `account/` 为企业侧根，挂载到官方命名空间之下。
>
> **Visibility 值域（企业侧扩展，SDK 层实现；OpenViking 官方无此字段）**：`public` / `shared_within_dept` / `shared_within_user` / `private` / `session_only` / `inherit`（沿用父级可见性）；SDK 层按值拼路径并做召回白名单过滤。
>
> **服务端记忆分类（官方，附件4 第 323-333 行）**：LLM 抽取后落地为 preferences / events / cases / memories / skills 五类 + archive 对话档案 + resources 多模态资源，共 7 类。

| 隔离级 | OpenViking 目录（企业侧扩展根） | 对应官方命名空间 | 记忆类型（官方分类） | 例子 | Visibility 值（企业侧） |
|---|---|---|---|---|---|
| 公司（全组织共享） | `account/resources/public/` | `viking://resources` | 多模态资源 / 知识规则 | 公司文档库、项目代码库、共享知识与规则 | `public` |
| 部门 | `account/resources/team/{dept}/` | `viking://resources`（企业侧分区） | 多模态资源 / skills | 设计部 SOP、研发部规范 | `shared_within_dept` |
| 个人（跨 Agent 共享） | `account/user/{uid}/memories/` | `viking://user` | preferences / events / cases / memories | 用户偏好、稳定事实、决策案例 | `shared_within_user` |
| 个人技能 | `account/user/{uid}/skills/` | `viking://user`（官方子目录） | skills | 用户习得的操作方式、任务经验 | `shared_within_user` |
| Agent 私有 | `account/agent/{aid}/private/` | `viking://agent` | memories / skills | Claude Code 项目缓存 | `private` |
| 会话（临时） | `account/agent/{aid}/session/{sid}/archive/` | `viking://session` | archive（对话档案） | 单次对话上下文（20000 token 窗口切分） | `session_only` |
| 子 Agent 隔离 | `account/agent/{child_aid}/session/{child_sid}/` | `viking://session`（官方 SubagentStart 分配） | archive（独立 peer_id 归档） | Task 派生子 Agent 的独立会话 | `session_only` |
| 归档冷存 | `account/archive/`（挂 TOS） | 企业侧扩展（180 天规则） | 全类型冷存 | 180 天未访问的长期记忆 | `inherit` |

> **resources/ 语义澄清（对齐附件4 第 31、333 行）**：官方 `viking://resources` 是"多模态资源：项目文档、代码库、知识/规则"，通过 MCP `add_resource` 主动加入。本报告把"公司文档库/项目代码库/共享知识与规则"挂到 `account/resources/` 之下是企业侧分区约定；HR 制度等企业策略性文本可另设 `account/resources/policies/` 分区，属企业侧约定，非官方语义。
>
> **子 Agent 隔离（官方能力，附件4 第 82-83 行）**：Claude Code 通过 Task 工具派生子 Agent 时，`SubagentStart` Hook 分配独立 `cc-<sha256(child_session_id)>` OV Session，子 Agent 以独立 peer_id 归档，避免主/子 Agent 记忆串染。这是 OpenViking 官方支撑多 Agent 平台的关键能力，对齐 §3 业务题 Q1 多 Agent 共享。
...
buildParts：text→text part，tool_use/tool_result→tool part（output ≤2000 字）
   │
   ▼
addMessage → POST 到 OV Session（cc-<sha256(cc_session_id)>）
   │
   ▼
...
模型判断"这一条值得沉淀" → 调 MCP store 工具
   │
   ▼
SDK 层补五级 ID + Visibility 拼路径（企业侧扩展）
   │
   ▼
...
输出 <openviking-context> 注入块 → 追加到用户 Prompt 前
   │
   ▼
SDK 层加 Visibility + dept + user 三路白名单过滤（企业侧扩展）
   │
   ▼
SDK 发 OTel Trace + Prometheus 指标（命中率 / 延迟 / llm_token_usage / embedding_token_usage）
```

**路径 4 · MCP search 模型主动检索（召回 · 模型意图驱动）**
```
模型判断"自动注入的 6 条不够" → 调 MCP search 工具（可指定 scope）
   │
   ▼
SDK 按 user_id + dept_id 过滤 + Visibility 白名单（企业侧扩展）
   │
   ▼
...
| 服务 | 数量 | 用途 | 归属 |
|---|---|---|---|
| openviking-server | 1 组（主 + 副本） | 记忆治理 + 抽取管线 + 检索 + 存储主服务 | 官方 |
| RAGFS 存储卷 | 1 组 | OpenViking 目录树承载（viking://{resources,user,session,agent}/…） | 官方 |
| web-studio | 1 个 | B 侧 Web 控制台 · Playground + 检索轨迹可视化（命中率调优利器） | 官方 |
| ov CLI（Rust） | 复用 | 一期运维：目录树 `ls/tree/read/search` + 批量导入 | 官方 |
| MCP Server（/mcp 端点） | 1 组 | 暴露 9 工具给模型按需调用 | 官方 |
| Plugins（Hooks 脚本） | 每 Agent 1 套 | 7 Hook 生命周期回调（auto-recall.mjs / auto-capture.mjs 等） | 官方 |
| 企业侧治理 SDK | 1 组 | 五级 ID 注入 + Visibility 拼路径 + 观测转发（无状态，与 Agent 部署或独立部署均可） | 企业侧扩展 |
| Prometheus + Grafana | 1 套 | 指标采集 + 面板（含 llm_token_usage / embedding_token_usage 成本曲线） | 官方 + 企业侧扩展 |
| KMS | 复用企业侧 | 静态加密密钥 + 租户密钥 | 企业侧扩展 |
| TOS（对象存储） | 复用企业侧 | archive/ 冷存（180 天规则）+ 原始附件 | 企业侧策略 |
| 模型层 | 外部调用 | LLM（extraction/compression）+ Embedding（向量化）+ VLM（多模态资源）+ doubao-seed-rerank | 官方配置 |

- **目录承载"公司 / 部门 / 个人 / 会话"**：`account/resources/team/{dept}/` + `account/user/{uid}/memories/` + `account/agent/{aid}/session/{sid}/archive/`
- **SDK / MCP 层**：统一 ID 注入、visibility 拼路径、观测埋点；详见 [一期落地方案](附件3-与openvking对接问题.md)
...
1. **业务题 Q1/Q3：四级隔离天然匹配** — OpenViking 官方 **4 顶级命名空间**（`viking://resources / user / session / agent`，附件4 第 28-35 行）天然映射"资源 / 个人 / 会话 / Agent"四类；本报告在此基础上叠加企业侧扩展（`account/` 多租户根 + `team/{dept}/` 部门分区 + `archive/` 180 天冷存）形成"公司 / 部门 / 个人 / 会话"四级隔离，无需改 OpenViking 源码（详见 §6.4.0 设计理念）
...
4. **业务题 Q4：企业级治理闭环** — 自托管 + KMS + 租户密钥 + audit log，配合 SDK 层强制 ID 注入与 Visibility 白名单，覆盖 ACL/审计/驻留/删除权四要素；具体合规证书由公司部署侧承担
