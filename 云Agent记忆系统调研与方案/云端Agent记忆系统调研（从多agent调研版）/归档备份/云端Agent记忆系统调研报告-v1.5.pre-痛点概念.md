...
- **无上下文 → 执行质量差**：Agent 不记得用户偏好、项目结构、部门 SOP，每次都从零开始，输出频繁偏离规范、重复试错
- **无 RAG 知识库 → 知识无法沉淀复用**：部门规范、项目文档、历史决策没有统一入库与语义检索入口，Agent 无法按需调取权威资料，只能靠用户手工粘贴
...
**目标形态****目标形态**：为云端 Agent 挂载**以「知识库」为中心的记忆系统**——知识库按归属与用途分类（个人偏好库 / 个人技能库 / 项目上下文库 / 部门 SOP 库 / 公司政策库 / 会话归档库，详见 §6.4.3），Agent 不拥有知识库、只被授权访问。具体具备：

- **按库分类承载**：短期（会话内）+ 跨会话工作 + 长期稳定 + 归档冷存，分别落在对应类型的知识库
- **隔离分两层**：
  - **规范库（逻辑隔离）**：个人偏好/技能/会话归档等，共享实例内以 namespace 隔离
  - **实例库（物理隔离）**：部门 SOP、公司政策等高敏/只读库，用独立 OpenViking 实例隔离（方式C，见 §6.4.0）
...
**问题一 · Agent 没有上下文**（使用侧）
- Agent 无记忆系统也无 RAG 知识库，跨会话失忆、跨 Agent 不复用、知识无入库入口 → 执行质量差、使用成本高
- 要回答：记忆怎么分层承载（短期/工作/长期/归档）、怎么自动沉淀与按需召回、怎么把部门文档 / SOP 入库供 Agent 检索
...
> **定义**：> **定义**：Agent 记忆系统是位于 **Agent Runtime 与知识/存储底座之间**的**上下文管理层**，由「记忆分层 + 写入判断 + 召回 + 生命周期治理 + 跨 Agent 共享 + 治理审计」几个部分组成，把会话中产生的事实、偏好、任务状态与可共享知识，沉淀为可写入、可检索、可治理、可审计的记忆，按用户/部门/Agent/会话边界安全召回。
>
> **组成部分简介**：
