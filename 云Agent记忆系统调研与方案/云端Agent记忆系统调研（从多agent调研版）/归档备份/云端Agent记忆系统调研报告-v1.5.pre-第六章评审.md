...
2000 人规模互联网公司统一部署多类 AI Agent，知识库类型多（个人偏好/技能、项目上下文、部门 SOP、公司政策、会话归档等 6 类，见 §6.4.3），且知识库天然需要被分享与复用。一期"1 实例↔1 Agent↔1 库"解决了单 Agent 记忆链路跑通的基础问题，但无法支撑以下真实业务需求，二期是这些需求落地的必要演进：
...
**一期架构图**（1 实例 ↔ 1 Agent ↔ 1 库 · 多 Agent 并存各 1-to-1）**一期架构图**（1 实例 ↔ 1 Agent ↔ 1 库 · 多 Agent 并存各 1-to-1）
...
4. **多知识库联邦查询延迟** — 方案3 多实例物理隔离部署下，Agent 跨知识库召回需平台层 fan-out 多个知识库实例并行检索再合并排序，增加召回 P95 延迟
...
| R3 | 2000 人规模的成本失控 | 算力/存储账单爆 | 引入"记忆生命周期"策略：180 天未访问自动归档到 TOS 对象存储（知识库级归档，对齐 §6.4.3）；并按官方默认关键参数（commit=20000 / recallLimit=6）预算 LLM+Embedding 成本 |
