...
- **迁移与授权**：一个 Agent 可访问多个知识库（跨库联邦查询，见 §6.4.0）；**一期按 1 Agent↔1 库的最简映射先跑通**，后续扩展为 1 Agent↔N 库
...
| ③ 知识库授权中心 + 平台路由层 | Agent 身份鉴权（mTLS/JWT）+ 知识库授权矩阵（RBAC + ACL，Agent×知识库 允许/不允许）+ 按授权路由到目标知识库实例 + 审计日志 + 观测埋点转发（含 token 成本）                    | 知识库授权中心（RBAC + ACL + 审计）                                      | 企业侧扩展      |
