...
│  ├────────────────────────────────────────────────────────┤    │
│  │ 目录承载：account/ 根下 resource/ team/ user/ agent/ archive/ │
│  ├────────────────────────────────────────────────────────┤    │
...
> **说明**：`account/` 是 OpenViking 内置的账号根节点（对应 ROOT/ADMIN/USER 权限模型），承载全公司账号元数据；本表所列 6 类目录均挂载于 `account/` 之下。图 6.4.1 中的 `account/` 层与本表其他 5 类目录是"根 vs 子"关系，不与 `user/ team/ agent/` 并列。
>
> **Visibility 值域**：`public` / `shared_within_dept` / `shared_within_user` / `private` / `session_only` / `inherit`（沿用父级可见性）；SDK 层按值拼路径并做召回白名单过滤。

| 隔离级 | OpenViking 目录 | 例子 | Visibility 值 |
|---|---|---|---|
| 公司（全组织共享） | `account/resource/public/` | 公司通用规范、HR 制度 | `public` |
| 部门 | `account/team/{dept}/` | 设计部 SOP、研发部规范 | `shared_within_dept` |
| 个人（跨 Agent 共享） | `account/user/{uid}/` | 用户偏好、稳定事实 | `shared_within_user` |
| Agent 私有 | `account/agent/{aid}/private/` | Claude Code 项目缓存 | `private` |
| 会话（临时） | `account/agent/{aid}/session/{sid}/` | 单次对话上下文 | `session_only` |
| 归档冷存 | `account/archive/`（挂 TOS） | 180 天未访问的长期记忆 | `inherit` |
...
OpenVikingAdapter → OpenViking HTTP/SDK
   ...
   └─→ 会话结束：session.commit() → SessionCompressorV2 → MemoryUpdater
                                                    │
                                                    ▼
                                       写入到 account/user/{uid}/ 或 account/team/{dept}/ 等目录
```
...
| AGFS 存储卷 | 1 组 | OpenViking 目录树承载（account/{agent,user,team,resource,archive}/…） |
...
- **目录承载"公司 / 部门 / 个人 / 会话"**：`account/team/{dept}/` + `account/user/{uid}/` + `account/agent/{aid}/session/{sid}/`
- **SDK / MCP 层**：统一 ID 注入、visibility 拼路径、观测埋点；详见 [一期落地方案](附件3-与openvking对接问题.md)

### 6.5 核心理由（5 条 · 对齐 §3 5 道业务题）

1. **业务题 Q1/Q3：四级隔离天然匹配** — OpenViking 的**分级目录**（`account/` 根节点下的 `resource/ team/ user/ agent/ archive/` 五类子树）可直接映射"公司 / 部门 / 个人 / 会话"四级 + 冷存，无需额外抽象层
2. **业务题 Q2：短期 + 长期记忆闭环** — OpenViking 内置 **L0/L1/L2 三层信息模型 + 官方抽取管线（Parser + TreeBuilder + SemanticQueue + SessionCompressorV2 + MemoryUpdater）**，从对话解析到长期入库无需自建
...
5. **业务题 Q5：运行可观测开箱可用** — OpenViking 原生 Prometheus 指标 + SDK 层 OTel Trace，命中率 / 污染率 / P95 / 成本四条曲线可开箱使用
