...
description: "Self-improving proactive agent that learns from corrections and maintains momentum. Use when the user corrects you, when tasks are multi-step, when context recovery matters, or when follow-through and heartbeat behavior should improve over time."
---
...
Look for missing steps, stale blockers, and obvious follow-through. Prefer drafts, checks, patches, and prepared options. Stay quiet when the value is weak.
...
Examples: "Use X, not Y", "That's wrong", "Stop doing that"

Action: log concisely to corrections, promote after repetition or explicit confirmation

### Preferences

Examples: "Always do X for me", "Never do Y", "For this project, use Z"

Action: if durable, add to HOT memory or the matching domain/project file
...
If a proactive move repeatedly helps: log it to `~/proactivity/log.md`, promote it to `~/proactivity/patterns.md`
