...
function validateStructure(courseDir) {
  var errors = 0, warnings = 0, fails = [], details = [];
  ...
  var assetsDir = path.join(courseDir, 'assets');
  if (fs.existsSync(assetsDir)) {
    var imgCount = fs.readdirSync(assetsDir).filter(function(f) {
      return f.match(/\.(png|jpg|jpeg|gif|webp)$/i);
    }).length;
    pass('assets/ 目录: ' + imgCount + ' 张图片');
  } else {
    warn('课程根目录无 assets/ 目录');
  }

  var rootMds = fs.readdirSync(courseDir).filter(function(f) {
    return f.endsWith('.md') && f !== 'course-info.md';
  });
  pass('课程目录下 ' + rootMds.length + ' 篇文章MD');

  return { pass: errors === 0, errors: errors, warnings: warnings, fails: fails, details: details };
}

module.exports = { validateFile: validateFile, validateStructure: validateStructure };
