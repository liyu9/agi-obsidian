...
- **降本增效**：通过分层加载与摘要，显著降低 token 消耗（对标官方 LoCoMo 数据，Claude Code + OpenViking token 降幅 -63.2%）

### 1.3 非目标（本期不解决）

- 跨 Agent 实例的记忆共享（每个 Docker 独立，不做打通）
- 跨用户的团队级共享知识库（未来独立立项）
- OpenViking 层的多租户 / RBAC / API Key 治理（Docker 天然隔离，暂不下沉到记忆层）


## 二、需求与约束

> 本章需求由步骤 1 调研的 Claude Code / Codex / Hermes / OpenClaw 四大 Agent 的一手记忆最佳实践反推得出。每条需求都附「共性证据」（≥ 2 Agent 支持）或标注「单点参考」；来源 URL 集中放到附录 C。
...
| F13 | **显式遗忘** | 支持按 URI/语义/时间自动+手动删除 | OpenClaw `memory_forget` / Codex `max_rollout_age_days` / Claude Context Editing |
...
| **成本** | 显著降低 token（对标 OpenViking Claude Code +OpenViking token -63.2%） | OpenViking README LoCoMo 表 |
...
| 阈值异步 commit | `pending_tokens > commitTokenThresholdRatio × tokenBudget` → `commit(wait=false)` | openclaw-plugin README「afterTurn」段 |
...
| NF9 | 成本 -60% | LoCoMo Claude Code 侧 token -63.2% | ✅ | OpenViking README |
...
**满足度小结**：25 项中 ✅ 15 项，⚠️ 10 项，❌ 0 项。所有缺口都可通过**通用记忆系统的 Adapter/治理层**补齐，不影响以 OpenViking 为后端的技术路线。
...
- `<openviking-context>` token budget 建议：模型上下文窗口的 10-20%
...
| `viking://user/<user_id>/agent/<agent_id>/memories` | 隔离级记忆 | 长期 | user × agent | `isolateUserScopeByAgent=true` 时 |
...
判断：
  ...
  2. 否则若有 sessionKey → 使用 sessionKey 派生
  3. 否则 → sha256(agent_native_id) 前 16 位 hex，前缀 <agent_type>-
输出：cc-xxx / cx-xxx / hm-xxx / oc-xxx
```
...
| Key Facts & Decisions | 关键事实与决策 | `user/<u>/memories` 或 `user/<u>/cases` |
| Files & Context | 涉及的文件、代码位置 | `user/<u>/memories`（tag: file） |
| Errors & Corrections | 错误与纠正 | `user/<u>/cases` |
...
- **异步 vs 同步阈值**：默认 `commitTokenThresholdRatio=0.6`（合理推算）；生产环境建议观测后调整

#### 4.4.4 与 4 Agent 写入对齐
...
- `<openviking-context>` token budget 默认 = 模型上下文窗口 × 10%
...
| **使用度过期** | `hit_count > 阈值` 则永久保留，反之衰减 | Adapter 计数 |
...
每条记忆条目 JSON schema（通用系统在与 OpenViking 通信时使用）：
...
interface AgentAdapter {
  ...
  peer_prefix: string     // 'cc' | 'cx' | 'hm' | 'oc' | 自定义
...
- 向后兼容：`provenance` 字段允许扩展；`category` 允许新增分类
- 向前兼容：Client SDK 在读取到未知字段时忽略而非报错
...
| `onSubagentStart/End` | §6.1 SubagentStop 兜底 | 子会话独立 session_id + peer_prefix |
