...
linear：线性核函数poly：多项式核函数rbf：高斯核函数（默认）sigmoid：sigmoid 核函数
...
![](assets/20260512-80712-01-头图.png)

数据表一共包括了 32 个字段，代表的含义如下：

![](assets/20260512-80712-02-图片1.jpg)
...
![](assets/20260512-80712-03-图片2.png)

首先我们需要加载数据源；在准备阶段，需要对加载的数据源进行探索，查看样本特征和特征值，这个过程你也可以使用数据可视化，它可以方便我们对数据及数据之间的关系进一步加深了解。然后按照“完全合一”的准则来评估数据的质量，如果数据质量不高就需要做数据清洗。数据清洗之后，你可以做特征选择，方便后续的模型训练；在分类阶段，选择核函数进行训练，如果不知道数据是否为线性，可以考虑使用 SVC(kernel=‘rbf’) ，也就是高斯核函数的 SVM 分类器。然后对训练好的模型用测试集进行评估。
...
![](assets/20260512-80712-04-图片3.png)

![](assets/20260512-80712-05-图片4.png)
...
![](assets/20260512-80712-06-图片5.png)
...
---
来源：极客时间
链接：https://time.geekbang.org/column/article/80712
日期：2026-05-12
