...
你 `cd /my-monorepo/services/user-service`。你启动 `claude`。**此时，Claude Code 会向上递归查找，自动加载并合并 (A) 和 (B) 两个上下文文件。** AI Agent 从一开始就知道全局规范和 `user-service` 的专属规范。

**场景二：在项目根目录下，分析 **`**order-service**`** 的文件**

你 `cd /my-monorepo`。你启动 `claude`。**此时，Claude Code 只加载了 (A) 项目根上下文。**然后，你向 AI 发出指令：`> @services/order-service/main.go "分析这个文件的逻辑"`。就在 AI Agent 读取 `main.go` 的这一刻，它会动态地发现并加载 © `order-service` 的专属上下文。于是，AI Agent 在分析 `main.go` 时，会同时知道全局规范和 `order-service` 的专属规范（比如订单 ID 必须是 ULID）。
...
- **即时生效：****当你保存并关闭编辑器后，Claude Code 会****自动重新加载**这些文件，确保你的修改在下一次交互中立即生效。
...
---
来源：极客时间
链接：https://time.geekbang.org/column/article/927870
日期：2026-05-07
