...
进入 `/hooks` 配置菜单。**事件：**选择 `PreToolUse`，因为我们要在操作发生之前进行拦截。**Matcher：**输入`Edit|Write|MultiEdit`，因为我们只关心有破坏性的写操作。
...
如何在同一个 Hook 命令中处理不同的文件后缀？（提示：`case` 语句或多个 `if-elif`）这个增强版的 Hook 命令应该是什么样的？

欢迎在评论区分享你的多语言自动化格式化脚本！如果你觉得有所收获，也欢迎你分享给其他的朋友，我们下节课再见！

---
来源：极客时间
链接：https://time.geekbang.org/column/article/931222
日期：2026-05-07
