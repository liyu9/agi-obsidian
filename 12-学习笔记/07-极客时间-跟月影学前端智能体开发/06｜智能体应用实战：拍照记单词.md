...
![](assets/20260512-871596-01-头图.png)
...
这个组件主要是实现图片上传的 UI 卡片样式，并通过`updateImage`事件将图片 Base64 内容传给父级组件。

```
const emit = defineEmits(['updateImage']);
```

获取图片 Base64 内容的原理，我们在前面的课程中已经介绍过，这里就不再重复了。

这样我们就实现了一个预览并上传图片的 UI 组件，在 App.vue 中我们可以引入并使用它。
...
![](assets/20260512-871596-02-图片1.png)
...
![](assets/20260512-871596-03-图片2.gif)
...
我们在其中添加语音配置项，配置 appId、accessToken、clusterId，然后我们将 vocieName 也作为配置项，选择 en_female_anna_mars_bigtts，这是火山引擎语音的音色中比较好听的一个英文人物音色。
...
![](assets/20260512-871596-04-图片3.png)
...
> GitHub 上完整的代码在 [https://github.com/akira-cn/frontend-dev-large-model-era/tree/main/capture_the_word]()
...
---
来源：极客时间
链接：https://time.geekbang.org/column/article/871596
日期：2026-05-12
