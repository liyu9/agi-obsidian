...
我们的 JSON 解析器实现，整体就是一个手写的基于**状态机词法解析（lexer）****加上****带栈自动机**的手写 JSON 解析器，它通过一个大分发函数 trace(input: string)，按照当前状态分类处理输入字符，最终解析完成后触发相应的事件。
...
![](assets/20260512-874806-01-头图.jpg)
...
当检测到某个状态对应的结构已经**完整**读完，就调用 reduceState 函数做一些**收尾操作**。
...
首先是**状态机。****我们为什么要给 JSON 定义这么多状态，而不是直接正则匹配一大串字符？因为 JSON 结构具有嵌套、递归特性，用状态机 + 栈可以****清晰地表示进入和退出不同层级**。
...
---
来源：极客时间
链接：https://time.geekbang.org/column/article/874806
日期：2026-05-12
