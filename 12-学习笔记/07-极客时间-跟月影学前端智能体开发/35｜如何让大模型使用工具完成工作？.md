...
大部分 OpenAI API 兼容的大模型，都可以通过**在请求时传递 tools 参数**来告诉大模型可以调用哪些工具。tools 的参数配置格式大部分大模型官网文档中都有，而且参数形式基本上都比较一致，比如 moonshot 调用 tools，我们可以直接在官网找到相关文档：[https://platform.moonshot.cn/docs/api/tool_use]()。
...
![](assets/20260512-889118-01-头图.png)

## 让大模型继续处理 tools 调用返回的结果

上面的例子是最简单的大模型调用工具返回结果的过程，我们可以用下面的图表示。

![](assets/20260512-889118-02-图片1.png)
...
![](assets/20260512-889118-03-图片2.png)
...
在 onmessage 里，我们判断一下，如果 event 类型是 tool_call，那么提示正在调用函数。注意这里有个细节是函数调用前，上一轮思考过程结束，但是函数调用完，还有下一轮推理。所以我们把上一轮思考结束时输出的多余 `</think>` 标记删掉，并在 UI 中显示提示信息 `调用工具: ${event.data}` 。
...
这样我们就处理了前端流式响应，剩下一些修改小细节，如果对这些有兴趣的同学，可以从[GitHub 仓库]()拉取代码自行研究。

最终代码运行结果如下：

![](assets/20260512-889118-04-图片3.gif)
...
![](assets/20260512-889118-05-图片4.jpg)

## 课后练习

封装和改进 server.ts，实现以下两个功能其中一个，或者同时支持两个：

让模型支持在对话时同时调用多个工具。让模型支持在对话时级联调用，比如先调用 DirFiles 查找文件，再调用 ReadFile 读取某个文件中的详细内容。
...
---
来源：极客时间
链接：https://time.geekbang.org/column/article/889118
日期：2026-05-12
